#!/usr/bin/env python3
# scanner.py
# Analizador léxico + Interfaz gráfica (un solo archivo)
# Contiene:
#  - Funciones core: generar_diccionario_desde_csv, cargar_diccionario, analizar_texto, guardar_salida
#  - Interfaz Tkinter para manejar todo el flujo visualmente
# No se ha modificado la lógica original; la GUI invoca esas funciones.
...
# ============================
# FUNCIONES CORE (tal como las diste)
# ============================

def generar_diccionario_desde_csv(csv_path, txt_output):
    """
    Genera diccionario a partir de un CSV que contenga columna "Alfabético".
    Escribe un archivo txt con una palabra por línea (hasta 1000).
    """
...
def cargar_diccionario(ruta):
    """
    Carga el diccionario (.txt) y devuelve un set de palabras.
    """
...
# ============================
# EXPRESIONES REGULARES (tal como las definiste)
# ============================

ER_PALABRA_BASICA = r'^[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+$'
ER_PUNTUACION = r'^[\.\,\;\:\¿\?\¡\!]$'
ER_DIGITO = r'^\d+$'

# ============================
# TOKENIZADOR / ANALIZADOR (tal como lo diste)
# ============================

def analizar_texto(texto, diccionario):
    """
    Analiza el texto usando el diccionario y las expresiones regulares.
    Devuelve lista de tuplas (TIPO, lexema).
    """
    texto = texto.lower()
...
def guardar_salida(tokens, ruta):
    """
    Guarda los tokens en el archivo de salida en el formato:
    TIPO LEXEMA (una pareja por línea)
    """
...
class AnalizadorGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Scanner - Analizador Léxico")
        self.geometry("980x640")
        self.minsize(920, 560)

        # Rutas por defecto (puedes cambiarlas desde la GUI)
        ...
        # Construir UI
        self.create_widgets()
        self.log("Interfaz lista. Lógica cargada.")
    ...
    def create_widgets(self):
        ...
        # --- Top: controles CSV y acciones rápidas
    ...
    # ---------- Operaciones en thread para no bloquear la UI ----------
...
# ============================
# MAIN: si se ejecuta el archivo, lanzar la GUI
# ============================

def main_cli_example():
    """
    Función auxiliar opcional: si se desea ejecutar el flujo sin GUI,
    mantener compatibilidad con el main original.
    (Pero al ejecutar scanner.py se lanzará la GUI por defecto.)
    """
    # 1. Generar diccionario desde CSV (si existe)
    ...
    if not os.path.exists("texto_entrada.txt"):
        print("No se encontró texto_entrada.txt. Crea el archivo o usa la GUI.")
        return
...
if __name__ == "__main__":
    # Si se pasa la bandera --cli se ejecuta el flujo por consola (compatibilidad).
    if "--cli" in sys.argv:
        main_cli_example()
    else:
        app = AnalizadorGUI()
